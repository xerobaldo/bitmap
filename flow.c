...
//Criação de uma nova estrutura do fluxo usando endereços de origem e destino, ainda está incompleto
...
unsigned long *wrapper(void);
unsigned long *

wrapper(void)
{
    unsigned long *bm = bitmap_allocate(2048);
    return bm;
}


//uint32_t *bmp = hash_bytes(flow->
